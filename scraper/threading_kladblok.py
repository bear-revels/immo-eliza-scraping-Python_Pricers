...
#immo_urls = Immoscraper.get_property_urls(1) #--> 403 seconds
# end_time = time.time()
...
def thread_properties():
    immo_urls = Immoscraper.get_property_urls(10)           #get urls from 10pages
    with ThreadPoolExecutor(max_workers=10) as executor:
        executor.map(Immoscraper.get_property_details, immo_urls)


# to compaire with and without threading:
def time_with_threading():
    start_time = time.time()
    immo_urls = Immoscraper.get_property_urls(10)           #get urls from 10pages
    ...
    #print("immo_urls are: ",immo_urls)
    print("Time with threading: ", time_with_threading)

def time_without_threading():
    start_time = time.time()
    immo_urls = Immoscraper.get_property_urls(1)           #get urls from 10pages

    for url in immo_urls:
        Immoscraper.get_property_urls(1)              #get url for 10 pages
